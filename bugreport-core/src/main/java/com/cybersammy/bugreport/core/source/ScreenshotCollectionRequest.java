...
public final class ScreenshotCollectionRequest {
    ...
    public record Entry(SourceProvenance provenance, RelativePath relativePath) {
        public Entry {
    ...
    public static ScreenshotCollectionRequest from(
            ...
            List<RelativePath> selectedImages) {
        ReviewedCollectionPlan reviewed = Objects.requireNonNull(plan, "plan");
        List<RelativePath> selected = List.copyOf(
        ...
        for (SourceProvenance provenance : screenshotSources) {
            selected.forEach(path -> entries.add(new Entry(provenance, path)));
        }
