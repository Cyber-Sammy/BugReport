...
to arbitrary manifest callers. The workspace sanitization coordinator issues it
only while publishing the exact final bytes produced by the trusted product
pipeline and recording explicit review. Production package planning therefore
has no bypass path. A plan still does not
